...
const TaxChannel = "" //ايدي روم ضرائب
...
client.on('interactionCreate', async interaction => {
    var member;
    let role = interaction.guild.roles.cache.find(role => role.id === ""); // ايدي رول اللى يستلم التكت
    if(interaction.isButton){

        if(interaction.customId === `idk`){
          if(interaction.member.roles.cache.some(role => role.id === "")) // حط هنا ايدي الرتبة اللى تستلم التكت
            {
        ...
        db.add(`alluser_${interaction.member.id}`, 1)
          ...
          const everyone = interaction.guild.roles.cache.find(r => r.name === "");//اسم رتبه اداره
          let l1 = interaction.user;
           ...
           collector.on('collect', async i => {
            ...
            if(interaction.member.roles.cache.some(role => role.id === "")) //ايدي رتبة اداره
              {
               ...
               interaction.channel.send({ content: `${role}` , embeds: [embed], components: [product]});
                const everyone = interaction.guild.roles.cache.find(r => r.name === ""); //اسم رتب اداره
...
interaction.channel.setName("ticket-" + interaction.user.username);                interaction.channel.permissionOverwrites.edit(role,{SEND_MESSAGES: true})
          ...
          interaction.channel.permissionOverwrites.edit('',{SEND_MESSAGES: false}) //ايدي رتبه يلي تقدر ترسل بعد احد يستلم تكت
          interaction.channel.permissionOverwrites.edit('',{VIEW_CHANNEL: true}) //ايدي رتبه يلي ماتقدر ترسل بعد احد يستلم تكت
          }else{
            interaction.followUp({ 
                content: `ماشفتك تستخدم الزر ؟`,
                ephemeral: true 
...
client.on(`messageCreate`, async message => {
  if(message.content.startsWith(prefix + "sendinfo")){
    ...
    let owner = ""; // ايدي اونر يلي يقدر يستعمل الامر
